...
const homecareSchema = z.object({
    ...
    postcode: z
        ...
        .regex(/^([A-Z][A-HJ-Y]?\d[A-Z\d]? ?\d[A-Z]{2}|GIR ?0AA)$/i, "Postcode must be a valid UK format (e.g. SW1A 1AA)"),
    country: z
        .string()
        .min(1, "Country is required")
        .refine(
            (value) => ["UK", "Scotland", "Wales", "Northern Ireland", "England"].includes(value),
    ...
    phone: z
        ...
        .regex(/^(\+44|0)7\d{9}$/, "Phone must be a valid UK mobile number (e.g. +447912345678 or 07123456789)"),
...
export default function EditHomecareModal({ homecareId, onClose, onSubmit }) {
        ...
        const fetchHomecareData = async () => {
            ...
            try {
                ...
                if (fetchedData.registrationDate) {
                    fetchedData.registrationDate = new Date(fetchedData.registrationDate).toISOString().split('T')[0];
                }
                ...
                showToast(err.response?.data?.message || "Failed to fetch homecare details.", "error");
                if (isMounted) {
                    onClose();
                }
            } finally {
    ...
    useEffect(() => {
        const file = watchImg && watchImg.length > 0 ? watchImg[0] : null;
        if (file instanceof File) {
            const newPreview = URL.createObjectURL(file);
            setEditImagePreview(newPreview);
            return () => URL.revokeObjectURL(newPreview);
        } else if (editImagePreview && editImagePreview.startsWith('blob:')) {
            // This case handles clearing the file input
            setEditImagePreview(null);
        }
    }, [watchImg, editImagePreview]);
        ...
        >
            ...
            >
                <div className="flex justify-between pt-2 items-center mb-4 border-b border-gray-200 dark:border-gray-700 pb-3 sticky top-0 bg-white dark:bg-gray-900 z-10">
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Edit Homecare</h2>
                    <button
                        onClick={onClose}
                ...
                {loading ? (
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input
                                    ...
                                    className={`mt-1 block w-full rounded-md border ${errors.homeCareName ? "border-red-500" : "border-gray-300"} shadow-sm p-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500`}
                                />
                                {errors.homeCareName && <p className="mt-1 text-xs text-red-500">{errors.homeCareName.message}</p>}
                            </div>
                            <div>
                                ...
                                <input
                                    ...
                                    className={`mt-1 block w-full rounded-md border ${errors.providerName ? "border-red-500" : "border-gray-300"} shadow-sm p-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500`}
                                />
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className={`mt-1 block w-full rounded-md border ${errors.registrationNumber ? "border-red-500" : "border-gray-300"} shadow-sm p-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500`}
                                />
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className={`mt-1 block w-full rounded-md border ${errors.serviceType ? "border-red-500" : "border-gray-300"} shadow-sm p-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500`}
                                />
                                {errors.serviceType && <p className="mt-1 text-xs text-red-500">{errors.serviceType.message}</p>}
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-400">Homecare Image</label>
                                {editImagePreview && (
                                    <div className="mt-2 mb-4">
                                        <img src={editImagePreview} alt="Current Homecare" className="w-32 h-32 object-cover rounded-md shadow-md" />
                                    </div>
                                )}
                                <input
                                    type="file"
                            ...
                            <div>
                                ...
                                <input
                                    type="tel"
                                    {...register("phone")}
                                    className={`mt-1 block w-full rounded-md border ${errors.phone ? "border-red-500" : "border-gray-300"} shadow-sm p-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500`}
                                />
                            ...
                            <div>
                                ...
                                <input
                                    type="email"
                                    {...register("email")}
                                    className={`mt-1 block w-full rounded-md border ${errors.email ? "border-red-500" : "border-gray-300"} shadow-sm p-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500`}
                                />
                            ...
                            <div>
                                ...
                                <input
                                    type="url"
                                    {...register("website")}
                                    className={`mt-1 block w-full rounded-md border ${errors.website ? "border-red-500" : "border-gray-300"} shadow-sm p-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500`}
                                />
                            </div>
                            <div>
                                ...
                                <input
                                    ...
                                    className={`mt-1 block w-full rounded-md border ${errors.contactPerson ? "border-red-500" : "border-gray-300"} shadow-sm p-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500`}
                                />
                            </div>
                            <div>
                                ...
                                <input
                                    ...
                                    className={`mt-1 block w-full rounded-md border ${errors.addressLine1 ? "border-red-500" : "border-gray-300"} shadow-sm p-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500`}
                                />
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    {...register("city")}
                                    className={`mt-1 block w-full rounded-md border ${errors.city ? "border-red-500" : "border-gray-300"} shadow-sm p-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500`}
                                />
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    {...register("postcode")}
                                    className={`mt-1 block w-full rounded-md border ${errors.postcode ? "border-red-500" : "border-gray-300"} shadow-sm p-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500`}
                                />
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    {...register("country")}
                                    className={`mt-1 block w-full rounded-md border ${errors.country ? "border-red-500" : "border-gray-300"} shadow-sm p-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 focus:ring-blue-500 focus:border-blue-500`}
                                />
                        ...
                        <div className="flex justify-end mt-8 gap-3">
                            <Button variant="ghost" onClick={onClose} type="button" disabled={isSubmitting} className={isSubmitting ? "opacity-50 cursor-not-allowed" : ""}>
                                Cancel
                            </Button>
                            <Button
                                type="submit"
                                ...
                                className={`transition-all duration-200 ${isSubmitting ? "bg-blue-400 text-white cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700 text-white"}`}
                            >
